...
void CG_PredictWeaponEffects( centity_t *cent ) {
	...
	// if it's not switched on server-side, forget it (but still send the appropriate angles for aimtype 4)
	if ( !cgs.delagHitscan ) {
		//CG_CreateAimtype4FireCmd();
		return;
	}
	
	// if we've opted out of unlagged
	if (cg_aimType.integer == 0)
	{
		return;
	}
	...
	if ( ent->weapon == WP_RAILGUN ) {
		...
		if ( cg_delag.integer & 1 || cg_delag.integer & 16 ) {
			...
			if ( cg_debugDelag.integer ) {
				// trace forward
				CG_Trace( &trace, muzzlePoint, vec3_origin, vec3_origin, endPoint, cent->currentState.number, CONTENTS_BODY|CONTENTS_SOLID|CONTENTS_PLAYERCLIP );

				// did we hit another player?
				if ( trace.fraction < 1.0f && (trace.contents & CONTENTS_BODY || trace.contents & CONTENTS_PLAYERCLIP) ) {
					...
					if ( cg.nextSnap ) {
						...
						//trap_SetUserCmdValue( c->currentState.clientNum + 1 , cg.zoomSensitivity );
						//CG_Printf("c->currentState.clientNum: %i -- c->currentState.clientNum + 1: %i\n", c->currentState.clientNum, c->currentState.clientNum + 1);
						//CG_CenterPrint( "Hit!", 380, SMALLCHAR_WIDTH );
						...
						//trap_SetUserCmdValue( c->currentState.clientNum + 1 , cg.zoomSensitivity );
						//CG_Printf("c->currentState.clientNum: %i -- c->currentState.clientNum + 1: %i\n", c->currentState.clientNum, c->currentState.clientNum + 1);
						//CG_Printf("c->nextState.clientNum: %i -- c->nextState.clientNum + 1: %i\n", c->nextState.clientNum, c->nextState.clientNum + 1);
						//CG_CenterPrint( "} else { Hit!", 380, SMALLCHAR_WIDTH );
		...
		//			for (i=0;i<32;i++) {
		//				hit[i] = cg.clientsHit & (1 << i);

		//				if (hit[i]) {
		//					cg.rawPacket.clientsHit[totalHit] = i+1;
		//					totalHit++;
		//				}
		//			}
		//		}
